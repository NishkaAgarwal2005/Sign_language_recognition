...
while True:
    ...
    #Time to configure the hand specifically into the ROI...
        ...
        if hand is not None:
            ...
            SAVE_DIR = r"C:\Users\nihaj\AppData\Local\Programs\Python\Python312\Scripts\gesture_data\test\8_l"

            # Create the directory if it doesn't exist
            if not os.path.exists(SAVE_DIR):
                os.makedirs(SAVE_DIR)

            if num_imgs_taken <= 18:
                if thresholded is not None:
                # Create the full save path
                    img_path = os.path.join(SAVE_DIR, f"{num_imgs_taken}.jpg")
        
                    # Save the image and check for success
                    if cv2.imwrite(img_path, thresholded):
                        print(f"Successfully saved image {num_imgs_taken}.jpg")
                    else:
                        print(f"Failed to save image {num_imgs_taken}.jpg")
                else:
                    print("Thresholded image is None. Skipping save.")
            else:
                break
            num_imgs_taken +=1
        else:
